...
*The start and end temporal elements are based on xref:metadatareference:core-attributes-table.adoc#modified[modified] timestamps for a metacard.*
====
...
Sorting by `date` will sort the results by the xref:metadatareference:core-attributes-table.adoc#effective[`effective`] date.
...
|`dtoffset`
|Integer > 0
|Specifies an offset (milliseconds), backwards from the current time, to search on the xref:metadatareference:core-attributes-table.adoc#modified[modified] time field for entries.
