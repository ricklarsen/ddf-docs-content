...
See xref:reference:/tables/Confluence_Federated_Source.adoc[Confluence Federated Source configurations] for all possible configurations.

[IMPORTANT]
====
If an additional attribute is not part of the Confluence metacard type or xref:developing:devcomponents/attribute-injection.adoc[injected], the attribute will not be added to the metacard.
====
