...
Each metacard result contains security attributes that are populated by the `CatalogFramework` based on the `PolicyPlugins` that populates this attribute. (These are not provided, so you must create your own plugin for your specific metadata.)
