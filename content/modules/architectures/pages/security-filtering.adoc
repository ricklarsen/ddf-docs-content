...
Metacard filtering is performed by the xref:architectures:filter-plugin.adoc[Filter Plugin] after a query has been performed, but before the results are returned to the requestor.

Each metacard result will contain security attributes that are populated by the `CatalogFramework` based on the `PolicyPlugins` that populates this attribute. (These are not provided, so you must create your own plugin for your specific metadata.)
