...
When an export operation is processed, the migration API does a look-up for all registered OSGi services
...
interface is included. In particular, the version string returned helps the migration API identify the version of the exported data from the corresponding migratable and must
be provided as a non-blank string.

When an import operation is processed when the current {branding} version matches the exported @{branding} version, the migration API does another look-up for all registered OSGi services that are
...
by the migratable or not. The `doMissingImport()` method is called instead of one of the other two methods when the migration API detects that the corresponding migratable data is missing from the
exported data. Any migratables that are tagged using the `OptionalMigratable` tag interface are automatically skipped unless otherwise specified when the import phase is initiated.

When an import operation is processed when the current {branding} version does not the exported @{branding} version, the migration API does a look-up for all registered OSGi services that are
implementing `Migratable` and call their `doVersionUpgradeImport()` method.

The services that implement the migratable interface are called one at a time based on their service ranking order, and do not need to be thread safe.
...
service. All other interfaces are implemented by the migration API that provides support for migratables.
...
migratable. The only noticeable requirement is that when the string compares equal using the `String.equals()` method, the migration API calls `doImport()` instead of `doIncompatibleImport()` to restore previously exported data for the migratable. +
...
Finally, the `doMissingImport()` method is called along with the context for the current import operation when data had not been exported for the corresponding migratable.
This is the case when a migratable is later introduced in the software distribution.
...
=== `OptionalMigratable`
This interface is designed as a tagged interface to identify optional migratables. An optional migratable is skipped by default during the import phase. It can still be manually marked as mandatory when initiating the import phase.
...
Entries are uniquely identified using a relative path and are specific to each migratable meaning that entries with the same path in two migratables do not conflict with each other. Each migratable is given its own context (a.k.a. sandbox) to work with.
...
The `store()` and `store(boolean required)` methods automatically copy the content of the corresponding file as part of the export making sure the file exists (if required) on disk otherwise an error is recorded. If the path represents a directory then all files recursively found under the path are automatically exported. +
The `store(PathMatcher filter)` and `store(boolean required, PathMatcher filter)` methods  automatically copy the content of the corresponding file if it matches the filter as part of the export making sure the file exists (if required) on disk otherwise an error is recorded. If the path represents a directory then all matching files recursively found under the path are automatically exported. +
The `store(BiThrowingConsumer<MigrationReport, OutputStream, IOException> consumer)` method allows the migratable to control the export process by specifying a callback consumer that is called back with an output stream where the data can be writen to instead of having a file on disk being copied by the migration API.
...
The `restore()` and `restore(boolean required)` methods automatically copy the exported content of the corresponding file back to disk if it was exported; otherwise an error is recorded. If the path represents a directory then all file entries originally recursively exported under this entry's path are automatically imported. If the directory had been completely exported using one of the `store()` or `store(boolean required)` methods then in addition to restoring all entries recursively, calling this method also removes any existing files or directories that were not on the original system. +
The `restore(PathMatcher filter)` and `restore(boolean required, PathMatcher filter)` methods  automatically copy the exported content of the corresponding file if it matches the filter back to disk if it was exported; otherwise an error is recorded. If the path represents a directory then all matching file entries originally recursively exported under this entry's path are automatically imported. +
The `restore(BiThrowingConsumer<MigrationReport, Optional<InputStream>, IOException> consumer)` method allows the migratable to control the import process by specifying a callback consumer that is called back with an optional input stream (empty if the data was not exported) where the data can be read from instead of having a file on disk being created or updated by the migration API. +
...
The `getOperation()` method provides the type of migration operation (export or import) currently in progress. +
...
The `wasSuccessful(Runnable code)` method allows code to be executed. It returns true if no new errors are recorded as a result of executing the provided code. +
The `wasIOSuccessful(ThrowingRunnable<IOException> code)` method allows code to be executed which can throw I/O exceptions which are automatically recorded as errors. It returns true if no new errors are recorded as a result of executing the provided code. +
The `hasInfos()` method returns true if at least one information message has been recorded so far. +
The `hasWarnings()` method returns true if at least one warning message has been recorded so far. +
The `hasErrors()` method returns true if at least one error message has been recorded so far. +
The `verifyCompletion()` method verifies if the report is successful and if not, it throws back the first recorded exception and attach as suppressed exceptions all other recorded exceptions.
...
The `getMessage()` method provides a message for the corresponding exception, warning, or info that is displayed to the administrator on the console.

=== `MigrationException`
An `org.codice.ddf.migration.MigrationException` should be thrown when an unrecoverable exception occurs that prevents the export or the import operation from continuing. It is also possible to simply record one or many exceptions with the migration report in order to fail the export or import operation
