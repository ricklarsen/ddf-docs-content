...
A resource reader is matched to the scheme or protocol, such as `file://`) in the URI of the resource specified in the request to be retrieved.
...
Site information includes the source's name, version, availability, and the list of unique content types currently stored in the source (for example, NITF).
