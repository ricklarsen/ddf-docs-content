...
See xref:integrating:endpoints/endpoint-intro.adoc[Endpoints] for descriptions of provided endpoints.
...
. Retrieve an instance of `CatalogFramework` from the OSGi registry. (Refer to xref:developing:devguidelines/osgi-basics[OSGi Basics - Service Registry] for examples.)

. Deploy the packaged service to {branding}.
(Refer to xref:developing:devguidelines/osgi-basics.adoc[OSGi Basics - Bundles].)
[NOTE]
====
