...
.Source Architecture
image::sources_architecture.png[Source Architecture]
...
. Create an OSGi descriptor file to communicate with the OSGi registry. (Refer to xref:developing:devguidelines/osgi-basics.adoc[OSGi Services].)
...
See the xref:managing:configuring/connecting-to-sources.adoc[existing Catalog Provider list] for examples of Catalog Providers included in {branding}.
